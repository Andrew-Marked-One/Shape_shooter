...
class Game {
	...
	void sMovement() {
		float speed = std::sqrt((m_playerConfig.S * m_playerConfig.S) / 2);
		...
		for (auto& e : m_entities.getEntities()) {
			if (e->cLifespan && e->cShape) {
				if (e->cLifespan->remaining > 0) {
					...
					fillColor.a = static_cast<int>(std::round(255 * fade));
					outlineColor.a = static_cast<int>(std::round(255 * fade));
	...
	void spawnEnemy() {
		...
		Vec2 e_pos;
		do{
			float ex = static_cast<float>(rand() % static_cast<int>(std::round(m_window.getSize().x - 2 * m_enemyConfig.SR + 1)) + m_enemyConfig.SR);
			float ey = static_cast<float>(rand() % static_cast<int>(std::round(m_window.getSize().y - 2 * m_enemyConfig.SR + 1)) + m_enemyConfig.SR);
			e_pos = { ex, ey };
	...
	void spawnSmallEnemies(const std::shared_ptr<Entity>& entity) {
		if (!entity->isSmallEnenmy()) {
			...
			for (size_t i = 0; i < points; i++) {
				...
				Vec2 vel = { std::sin(current_a) * speed, -std::cos(current_a) * speed };
	...
	void spawnSpecialWeapon() {
		if (m_player->cInput->shootSpecial || m_currentFrame - m_lastSpecialWeaponUseTime < m_spacialWeaponRows) {
			...
			for (int i = 0; i < numOfBull; i++) {
				float x = std::cos(i * incriment + a);
				float y = std::sin(i * incriment + a);
				Vec2 targ = {x, y};
